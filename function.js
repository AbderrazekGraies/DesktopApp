...
async function desplaytypestr(types){
    ...
    for (const type of types){
        ...
        const data = await res.json();
        console.log(data);
        strongAgainst.push(...data.damage_relations.half_damage_from.map(t =>  t.name));
        weakAgainst.push(...data.damage_relations.double_damage_from.map(t => t.name));
    }
